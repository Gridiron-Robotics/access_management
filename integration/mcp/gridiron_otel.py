...
def _resolve_level(level: int | str | None) -> int:
    if isinstance(level, int):
        return level
    name = (level or os.environ.get("OTEL_LOG_LEVEL") or "ERROR").strip().upper()
...
def setup_observability(
    ...
    *,
    ...
    try:
        ...
        _log.info("gridiron_otel: SDK unavailable, observability disabled (%s)", exc)
        _STATE.update(done=True, status=status)
        return status
    ...
    try:
        ...
        tp.add_span_processor(
            SimpleSpanProcessor(span_exp) if forced else BatchSpanProcessor(span_exp)
        )
    ...
    try:
        ...
        lp.add_log_record_processor(
            SimpleLogRecordProcessor(log_exp) if forced else BatchLogRecordProcessor(log_exp)
        )
        set_logger_provider(lp)
        handler = LoggingHandler(level=_resolve_level(level), logger_provider=lp)
    ...
    if app is not None:
        try:
            ...
            _log.info("gridiron_otel: FastAPI instrumentation skipped (%s)", exc)
    try:
...
def traceparent_headers(carrier: dict[str, str] | None = None) -> dict[str, str]:
